...
fn the_grasp_gives_the_feet_back_the_frame_it_ends() {
    ...
    // Not `left + 1` frames: contact freeze pauses the countdown, and a Grasp
    // lands four arms and four freezes. The claim is about the frame the hold
    // *ends*, not about which frame that turns out to be.
    let mut ran_out = false;
    for _ in 0..(left as u32 + 60) {
        run(&mut w, 1, 0, 0);
        if !matches!(w.players[1].action, Action::Held { .. }) {
            ran_out = true;
            break;
    ...
    assert!(ran_out, "the hold never ended: {:?}", w.players[1].action);
    assert!(
...
fn reaches(button: u16, ahead: f32, across: f32, ducking: bool) -> bool {
    ...
    let park = |w: &mut World| {
        w.players[1].pos = sim::V3::new(base.x.add(m(ahead)), base.y, base.z.add(m(across)));
    };
    park(&mut w);
