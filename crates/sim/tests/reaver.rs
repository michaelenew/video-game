...
fn being_hit_throws_the_remembered_press_away() {
    ...
    let mut w = duel();
    let at_her = Input::aimed(L, Input::QUARTER_TURN * 2);
    // Executioner and the right click in one breath, because a *remembered*
    // press is the only state this rule is about. The send cuts a recovery
    // short and nothing else, so one thrown into a startup waits -- and it is
    // what happens to it while it waits that is being pinned here.
    //
    // A bare press proves nothing: she is free on the frame she gives it, so it
    // is spent immediately and there is never anything for the hit to throw
    // away.
    w.advance([Input::new(E | R), at_her]);
    assert!(
        matches!(w.players[0].action, Action::Startup { .. }),
        "she was free when she pressed, so there was nothing to remember"
    );
    assert!(
        w.players[0].shadow_queued > 0,
        "the press was not remembered at all"
    );
    assert!(
        !shadow(&w).is_out(),
        "the press was spent on the frame it happened, wind-up and all"
    );

    // The dummy's poke arrives during those wind-up frames.
    let mut hit = false;
    for _ in 0..20 {
        w.advance([Input::default(), at_her]);
    ...
    assert!(hit, "the dummy never managed to hit her during the wind-up");
    // One more frame: hits land after both fighters have stepped, so the first
    // read of her buttons that can see the stun is the next one.
    w.advance([Input::default(), at_her]);
    assert!(
        w.players[0].action.stunned(),
        "the stun was over before the memory could be tested against it"
    );
    assert_eq!(
        w.players[0].shadow_queued, 0,
        "she was hit and the press was still armed"
    );
    run(&mut w, 90, 0, 0);
    assert!(
        !shadow(&w).is_out(),
        "a right click pressed before she was hit sent the second body away \
         once the stun ran out -- on an input she gave in a situation that no \
         longer exists"
    );
}
