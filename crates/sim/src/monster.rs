...
pub struct Monster {
    ...
    pub hit_used: bool,
    /// Frames of contact freeze left, exactly as a fighter carries one.
    ///
    /// The creature has to freeze for the same reason both fighters do: the
    /// freeze is only fair because **both** ends of a blow pay it. Freeze one
    /// side and it stops being punctuation and becomes frame advantage, handed
    /// to whichever party was left running. Leaving the animal out of it made
    /// the hunt unwinnable -- the hunter lost a handful of frames on every
    /// exchange and the creature lost none.
    pub hitlag: u16,
...
impl Monster {
    ...
    pub fn step(&mut self, quarry: &[Quarry]) {
        ...
        // Frozen by contact. Nothing advances -- not the action, not the brain,
        // not the turn. See `hitlag`.
        if self.hitlag > 0 {
            self.hitlag -= 1;
            self.speed = Fx::ZERO;
            self.yaw_rate = Fx::ZERO;
