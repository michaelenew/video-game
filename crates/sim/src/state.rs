...
pub struct Player {
    ...
    pub space_held: bool,
    /// Buttons held at any point during the contact freeze currently running.
    ///
    /// A freeze is four to eight frames of a fighter's input being ignored, and
    /// a button tapped inside one used to be thrown away entirely -- the
    /// Champion's "we are settling this in the air" leap is pressed during
    /// exactly that window, and it was being swallowed. These are handed back on
    /// the first frame that is not frozen, which is what every fighting game
    /// does with the frames a hit takes away from you.
    ///
    /// Part of the snapshot, like the press edges it feeds, because rollback
    /// re-runs these frames and has to reach the same answer.
    pub buffered: u16,
    ...
    /// knowable.
    pub slam: Fx,
    /// Upward speed banked by a grab, cashed in when the hold ends.
    ///
    /// A move that both launches and holds cannot do the two at once: a held
    /// fighter is pinned to their captor by `drag_the_held`, which sets their
    /// velocity to zero every frame, so a launch handed to them on contact is
    /// overwritten before it moves anything. The Champion's uppercut is exactly
    /// that move -- its kit entry reads "launch, and hold on" -- and without
    /// this it did neither: the pair rose on the attacker's own `self_lift` and
    /// the victim was then released motionless at the top of it.
    ///
    /// So the launch waits. Held, you go where your captor goes; let go, and
    /// you are thrown.
    pub thrown: Fx,
    /// Hits taken without getting a turn back, for combo decay.
    ///
    /// Reset the moment this fighter is free to act again, so it counts a
    /// *string* rather than a round. See `tuning::combo_decay`.
    pub combo: u16,
    ...
    /// for anyone; it is the punctuation that makes a hit land rather than
    /// merely register. See `docs/design/stun.md`.
    pub hitlag: u16,
...
impl World {
    ...
    fn reset_positions(&mut self) {
        ...
        for (i, p) in self.players.iter_mut().enumerate() {
            ...
            *p = Player {
                pos: V3::new(Fx::from_int(side), GROUND_Y, t::mark_depth()),
                facing: V3::new(
    ...
    pub fn advance(&mut self, inputs: [Input; MAX_PLAYERS]) {
        ...
        //
        // `seen` is the players as they stood at the top of the frame, which is
        // exactly the comparison wanted and already on hand for the aiming.
        ...
        for attacker in 0..MAX_PLAYERS {
            ...
            if let Some(hit) = resolve_hit(&snapshot[attacker], &snapshot[defender], attacker as u8)
            {
                ...
                let freeze = apply_hit(&mut self.players[defender], hit, self.monster.is_none());
                // Both sides of a contact freeze together. Reading the count
                // back from the blow rather than recomputing it is what
    ...
    pub fn checksum(&self) -> u64 {
        ...
        for p in &self.players {
            ...
            h.write_u32(p.buffered as u32);
            h.write_u32(p.leap_used as u32);
            h.write_i32(p.slam.raw());
            h.write_i32(p.thrown.raw());
            h.write_u32(p.combo as u32);
...
/// Land a blow: the freeze, the stun, the shove.
///
/// Returns the frames of contact freeze it caused, so a caller that is itself a
/// fighter can freeze for the same number.
///
/// The order matters. Damage lands **first**, so the swell below reads the
/// victim's health as it is *after* the blow -- a hit therefore scales partly
/// by its own weight, which is what makes the last hit of a round the one that
/// sends a body across the arena. Then the stun, which interrupts. Then the
/// knockback, swelled by the damage they have already taken and divided by what
/// they weigh. See `docs/design/stun.md`.
pub(crate) fn apply_hit(defender: &mut Player, hit: Hit, duel: bool) -> u16 {
    // **A freeze is not invulnerability.** A second blow arriving during one
    // lands like any other and re-freezes; the longer of the two freezes wins,
    // so a fresh hit can never cut short a crunch already running.
    //
    // The alternative -- frozen fighters cannot be hit -- is tempting because it
    // trivially stops a fast field from locking somebody down, and it is wrong:
    // half the abilities in the game land several contacts inside a handful of
    // frames on purpose. A Grasp is four arms and a lotus is six blades, and
    // under that rule each of them became a one-hit ability that still claimed
    // to have caught you with all of them. What actually keeps a field honest is
    // its own cadence, which `no_source_of_damage_can_stun_for_longer_than_it_
    // takes_to_repeat` holds to be longer than the hold it applies.
    let freeze = hitlag_frames(hit.damage);
    defender.hitlag = defender.hitlag.max(freeze);
    ...
    if hit.blocked {
        ...
        let shove = hit.knockback.div(defender.class.weight());
        defender.stun_total = hit.blockstun;
        defender.action = Action::BlockStun {
            left: hit.blockstun,
        };
        defender.vel.x = hit.dir.x.mul(shove);
        defender.vel.z = hit.dir.z.mul(shove);
    } else {
        defender.health = (defender.health - hit.damage).max(0);
        // One multiplier for the whole launch vector, sideways and upward
        // alike, so a move keeps the angle it was authored with and only the
        // distance grows.
        let carry = knockback_swell(defender, hit.damage, duel).div(defender.class.weight());
        let shove = hit.knockback.mul(carry);
        defender.vel.x = hit.dir.x.mul(shove);
        defender.vel.z = hit.dir.z.mul(shove);
        if hit.grabs > 0 {
            defender.seized(hit.by, hit.grabs, 0);
        } else {
            let stun = hitstun_frames(defender, hit.hitstun, duel);
            defender.combo = defender.combo.saturating_add(1);
            defender.stun_total = stun;
            defender.action = Action::HitStun { left: stun };
            // The rest of the interrupt. An aerial's hang and a jump's sustain
            // are things the victim was in the middle of spending, and a hit
            // ends them the same way it ends a move. The crouch goes too, so
            // anything reading this fighter before their next tick -- a field
            // resolving later in the same frame, the renderer -- sees someone
            // who has been hit rather than someone still ducking.
        ...
        // Swelled and divided by weight like the sideways half: the two are one
        // launch vector, and scaling only one of them would bend every move's
        // angle as the round went on.
        let lift = hit.launch.mul(carry);
        if hit.grabs > 0 {
            // A move that both launches and holds cannot do the two at once:
            // `drag_the_held` is about to zero this fighter's velocity every
            // frame until the hold ends, so the launch is banked and spent on
            // release instead. See `Player::thrown`.
            defender.thrown = lift.max(Fx::ZERO);
...
/// a heavy move scales harder against a hurt opponent than a poke does. Without
...
///
/// `duel` gates it. See [`hitstun_frames`] for why.
fn knockback_swell(victim: &Player, damage: i32, duel: bool) -> Fx {
    if !duel {
        return Fx::ONE;
    }
    let max = t::max_health().max(1);
...
///
/// **The swell is a duel mechanic, and `duel` is what says so.**
///
/// It is a function of one fighter's share of their own bar, and it is only
/// fair because the other fighter has a bar the same size and is running along
/// it at the same rate. Take turns being hurt and you take turns being
/// launchable; that symmetry is the whole reason it reads as an arc rather than
/// as a punishment for being behind.
///
/// A hunt has no such symmetry. The creature carries ten times the health, so
/// it barely moves along its own bar while the hunter runs the length of
/// theirs. The mechanic becomes a one-way ratchet: every hit you take makes the
/// next one hold you longer and throw you further, against something that is
/// never held longer or thrown further in return. Measured, it took the
/// scripted hunt from winnable to unwinnable -- the hunter spent the fight
/// being thrown off the creature and walking back to it.
///
/// So in a hunt a blow does exactly what the move table says. The creature's
/// presence is the condition rather than a separate flag, the same way
/// `effects_reach` decides whether fighters can hurt each other.
fn hitstun_frames(victim: &Player, base: u16, duel: bool) -> u16 {
    if !duel {
        return base;
    }
    // The swell fades through a string. Each hit taken without getting a turn
    // back keeps less of it, so a combo runs out of the extra frames that were
    // letting it link -- and what is left is the move table's own number, which
    // a hit is always worth. See `tuning::combo_decay`.
    let mut left = t::swell_hitstun();
    for _ in 0..victim.combo.min(16) {
        left = left.mul(t::combo_decay());
    }
    let swell = Fx::ONE.add(hurt(victim).mul(left));
...
fn countdown(p: &mut Player, want_guard: bool) -> Option<Action> {
    Some(match p.action {
        ...
        Action::Held { .. } => {
            p.held_by = NOBODY;
            // Let go, and whatever the move banked is spent now. A grab that
            // set somebody down exactly where it picked them up is a grab that
            // only ever cost them frames.
            if p.thrown.raw() > 0 {
                p.vel.y = p.thrown;
                p.grounded = false;
                p.thrown = Fx::ZERO;
        ...
        Action::BlockStun { .. } | Action::HitStun { .. } | Action::Stagger { .. } => {
            // Free to act again, so the string is over and the swell is whole
            // for whatever lands next.
            p.combo = 0;
            Action::Free
        }
...
fn step_player(
    ...
    // Contact freeze, before anything else. Nothing about this fighter advances
    // while it runs -- not the move they are in, not the stun they are in, not
    // gravity -- so a blow reads as a collision rather than as a number going
    // down. Both sides get the same frames, so neither gains tempo by it, and
    // the renderer needs no part of it: poses are simulation state, so a frozen
    // fighter is a frozen picture for free.
    if p.hitlag > 0 {
        p.hitlag -= 1;
        // Nothing is acted on, but nothing is thrown away either. See
        // `Player::buffered`.
        p.buffered |= input.bits;
        return;
    }
    // The first unfrozen frame gets whatever was pressed during the freeze,
    // once. The aim is not replayed with it: you point where you are pointing
    // now, and only the buttons were owed to you.
    let input = if p.buffered != 0 {
        let owed = Input {
            bits: input.bits | p.buffered,
            ..input
        };
        p.buffered = 0;
        owed
    ...
    if let Some(drive) = shadow::dash_drive(p) {
        ...
        // **In the air, a stunned fighter still steers.**
        //
        // This is the game's answer to Smash's directional influence, and it is
        // the air control the player already has rather than a second mechanic
        // bolted alongside it. The Quake projection does exactly the job DI
        // does: pointing the way you are already being sent buys nothing, and
        // pointing across your motion turns the vector without spending it --
        // so a launched fighter picks *where* they land and never how far.
        //
        // What a hit takes away is everything that makes you dangerous: the
        // move, the jump, the dodge, the guard. What it leaves is the one tool
        // that only repositions. Being launched is therefore a trade rather
        // than a sentence -- you are held longer, and you get your steering
        // back to use while you are held.
        //
        // Grounded stun keeps none of this. Skidding along the floor is not a
        // moment anyone is flying through, and hitstun on the ground is the
        // straightforward tax it has always been.
        if steering && !p.grounded && matches!(p.action, Action::HitStun { .. }) {
            steer_while_stunned(p, move_dir(p.aim(input), ax, az), mob.air_speed);
        }
...
/// Air control at knockback speed: it turns you, it does not slow you.
///
/// `air_accelerate` ends with the air speed cap, which is a rule about how fast
/// a fighter may make *themselves* go. Knockback is not something they made
/// themselves do, and it routinely exceeds that cap, so letting the cap apply
/// while stunned would turn steering into a brake -- hold any direction and the
/// hit you just took would be halved. Pressing a direction must never be worth
/// less knockback than pressing nothing.
///
/// So the speed is put back afterwards, along the **new** heading. The turn
/// survives, the distance does not change, and below the cap this does nothing
/// at all -- ordinary air movement is untouched.
fn steer_while_stunned(p: &mut Player, wish: V3, wish_speed: Fx) {
    let before = V3::new(p.vel.x, Fx::ZERO, p.vel.z).flat_len();
    air_accelerate(p, wish, wish_speed);
    let after = V3::new(p.vel.x, Fx::ZERO, p.vel.z).flat_len();
    if after.raw() > 0 && before.raw() > after.raw() {
        let restore = before.div(after);
        p.vel.x = p.vel.x.mul(restore);
        p.vel.z = p.vel.z.mul(restore);
    }
}
...
fn settle(p: &mut Player) {
    ...
    // swell there is -- cutting it short at exactly that moment would throw
    // away the most legible thing the system does.
    if p.hitlag > 0 {
        p.hitlag -= 1;
        return;
    }
...
impl World {
    ...
    fn echo_cuts_the_other_fighter(&mut self, owner: usize, ghost: &Player) -> bool {
        ...
        hit.parried = false;
        apply_hit(&mut self.players[target], hit, self.monster.is_none());
        true
    }
...
impl World {
    ...
    fn drain(&mut self, victim: usize, effect: &Effect) {
        ...
        // Through `apply_hit` like everything else that hurts, so a tick stuns,
        // interrupts and shoves. A field that only took health would be a
        // number going down in the corner of the screen; one that takes the
        // turn away is *a piece of ground you have been denied*, which is what
        // a terrain class is supposed to be doing.
        //
        // Never blocked and never parried: guard is a facing arc and you cannot
        // face a floor. That is the same line `cut` draws from the other side --
        // a thrown blade is an attack and guard answers attacks, a field is not
        // and it does not.
        //
        // The shove points **away from the effect**, so standing in one costs
        // the spot as well as the health. Dead centre has no "away", so it
        // falls back to the victim's own facing rather than to nowhere.
        let out = V3::new(
            self.players[victim].pos.x.sub(effect.pos.x),
            Fx::ZERO,
            self.players[victim].pos.z.sub(effect.pos.z),
        );
        let dir = if out.flat_len().raw() == 0 {
            self.players[victim].facing
        } else {
            out.normalized()
        };
        let (hitstun, knockback) = effect.kind.field_stun();
        apply_hit(
    ...
    fn trade_with_the_creature(&mut self) {
        ...
        if let (Doing::Active { kind, .. }, false) = (beast.doing, beast.hit_used) {
            ...
            for i in 0..MAX_PLAYERS {
                ...
                // The animal pays the freeze too. It is only fair because both
                // ends of a blow stop for it; a creature that kept running
                // while its victim was frozen would collect those frames as
                // free advantage on every swing it threw. Longest wins, so
                // catching two fighters at once freezes it for the heavier of
                // the two rather than the last one checked.
                beast.hitlag = beast.hitlag.max(freeze);
                if parried {
        ...
        for i in 0..MAX_PLAYERS {
            ...
            // Both ends again, the other way round: cutting the animal stops
            // the pair of you for a beat. Without it a fighter could keep
            // swinging through a freeze the creature was paying and the whole
            // thing would read as one-sided, which is exactly what it would be.
            let freeze = hitlag_frames(dealt);
            beast.hitlag = beast.hitlag.max(freeze);
            self.players[i].hitlag = self.players[i].hitlag.max(freeze);
