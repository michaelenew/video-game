...
impl Class {
    ...
    ///
    /// These used to be `const` ratios written here, next to the prose
    /// justifying them -- while the Oven, the baked file and the palette all
    /// carried a second copy of the same numbers that nothing ever read. The
    /// sliders moved and the game did not. Reading the store is what makes them
    /// real; the values are unchanged.
    ///
