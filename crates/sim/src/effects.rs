...
impl EffectKind {
    ...
    /// Stun and shove from one tick of a field that is stood in.
    ///
    /// Short on purpose, and shorter than the tick interval: a field that held
    /// you for longer than its own cadence would land its next tick on someone
    /// who never got to move, and the player who laid it would not have to be
    /// there for any of it. Pinned by a feel test.
    ///
    /// Only the two that are *places* answer here. Everything else in this enum
    /// travels, and a travelling effect is an attack -- it goes through `cut`
    /// with the move's own hitstun and knockback, and can be blocked.
    pub fn field_stun(self) -> (u16, Fx) {
        match self {
            EffectKind::FirePillar => (t::pillar_hitstun(), t::pillar_knockback()),
            EffectKind::BlackSpike => (t::spike_hitstun(), t::spike_knockback()),
            _ => (0, Fx::ZERO),
        }
    }
