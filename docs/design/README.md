...
| [Bulwark](kits/bulwark.md) | Shield position — **and its weight**: stored when blocked, spent by Slam | `L` Bash · `M` Slam · `Q` Grapple · `R` Guard · `E` Throw/Recall/leap | New; [v2 being built](bulwark-v2.md), M1–M3 unplayed |
| ~~Gatekeeper~~ | — | — | Retired |
...
| [bulwark-v2.md](bulwark-v2.md) | v2: the shield is a battery — weight, Slam on `M`, the planted wall | **Being built** — M1 (weight, health table), M2 (Slam on `M`) and M3 (the loaded throw, the planted wall) built, unplayed |
