...
| **Double jump** | Space while airborne does nothing. The airdodge is currently the only air commitment. ⚠️ **Sharper since 2026-09-14:** `E` off the floor used to raise a stone, which made a stone under your own feet a sort of second jump for one class. It is Landfall now, so the Elementalist has lost the only thing in the game that was answering this question by accident. **And since 2026-09-17 the answer is enforced rather than merely intended:** an erupting stone was reporting a fighter who had already jumped off it as grounded again, which handed the still-held jump button a second takeoff and a second stone a third — one press reached twenty-six metres. `sim/tests/stones.rs` pins it |
| Dual mage | **⚠️ Since 2026-09-17 she has movement**: a 1.3 m step on every auto, forward on the dark one and backward on the light one, because the two already disagree about which way everything goes; the longest aerial hang in the game; and feet off the floor at depth or in ascension — faster, floating, wings in the colour of the force she carries. The numbers to watch are the step (1.3 m on a punch thrown every second is a lot of ground) and whether the light auto's backward version feels like being pushed around by your own attack. Naming the two forces. Ascension drain, refund, threshold and stun numbers. **Since 2026-09-16** the core is rebuilt — depth scales everything, the autos pull and push, Lance is two moves on middle click and Judgement earns its status through power — so the open questions moved with it: whether a four-to-one spread between the centre and the edge is the right one, whether "the form is the arm you last punched with" is findable by anybody who was not told, and whether a Judgement at full depth is too much of a health bar. See [kits/dual-mage.md](kits/dual-mage.md) |
...
| Elementalist | **⚠️ Since 2026-09-17 the structure jump is a third shorter and the double is a fraction of what it was**, most of which was a bug rather than a design: an erupting stone re-grounded a fighter who had already jumped off it, so one held button bought two takeoffs and two stones bought three. What is left is one press stacked on to the carry, and the eruption slowed by two frames on top of that. Whether the double still pays enough for its three-frame window is the open half. Structure cap of three is a readability guess, not a balance one — and **Landfall is a second way to spend it**, so it is under more pressure than when the guess was made. Stones are solid and standable, and Raise now places one where the crosshair is; the mobility that implies waits on moves that launch them. Her air row is built and none of its numbers have been played: the three to watch are in [kits/elementalist.md](kits/elementalist.md) §"Open questions" |
| Blood mage | Health cost flat or percentage; is 1.4x against a disabled enemy the right bonus. And **⚠️ newly the biggest question on the class, 2026-09-17: which movement is hers.** She had none, which was survivable while a full hop reached seven and a half metres and is not now the jump has been cut back to make room for class techniques. Two answers are built behind live Oven flags — the Grasp hauling her to a wall it caught instead of a person (on), and her dodge as a flat blink (off) — so the four combinations are two sliders away. Six more ideas, and the rule all of them were judged against, are in [kits/blood-mage.md](kits/blood-mage.md) §Movement |
