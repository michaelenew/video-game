...
**The Shadow Reaver joined it on 2026-09-13**, for a different reason worth stating because
the two are easy to confuse. Her mechanic is emphatically a state — where the second body is
— so `E` still changes it. What it is not is an *instant*: the shadow travels, and calling it
back is an attack that cuts and slows everything on the way home. An instant cannot have a
startup somebody reads, a damage number or a slow, and this needs all three. So the same
fourth slot, for the opposite reason: not "the mechanic has no state", but "changing it is a
move".
