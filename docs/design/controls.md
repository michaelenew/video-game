...
**A click is what disambiguates shift.** Shift with a click is the committed version of that
attack and shift with only a direction is a dodge, and the click is checked first — so holding
a direction while pressing shift + click still gives you the ability, and you keep moving
during it at whatever the move's own hindrance allows.

⚠️ This used to be the rule *"shift beats WASD when both are held"*, which guaranteed a
move-while-casting option always existed. **Dodge moving onto shift retired it**, and what it
guaranteed has not been replaced: see [Open](#open-since-the-dodge-moved).
